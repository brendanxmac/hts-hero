...
export const getNonExceptionTariffs = (
  countryCode: string,
  htsCode: string
) => {
  const inclusionTariffs: TariffI[] = TariffsList.map((tariff) =>
    getApplicableInclusions(tariff, countryCode, htsCode)
  ).flat();

  // Get all exception codes from all tariffs in the list
  const exceptionTariffCodes = new Set<string>();
  inclusionTariffs
    .filter((t) => t !== undefined) // TODO: why would there be undefined?
    .forEach((tariff) => {
      if (tariff.exceptions) {
        tariff.exceptions.forEach((exceptionCode) => {
          exceptionTariffCodes.add(exceptionCode);
        });
      }
    });

  // Filter out tariffs whose codes are in the exceptions list
  return inclusionTariffs.filter(
    (tariff) => !exceptionTariffCodes.has(tariff?.code)
  );
};
...
export const TariffsList: TariffI[] = [
  ...
  },
  {
    ...
    name: "Articles the product of China from 20 (e) and (f) (Section 232)",
    general: 25,
    special: 0,
    other: 0,
    inclusions: {
      countries: ["CN"],
      codes: [
        // Does not account for 98 exemptions
        // 20(e) just points to f
        // 20(f)
        "0203.29.20",
        "0203.29.40",
        "0206.10.00",
