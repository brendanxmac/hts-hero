...
import { elementsAtClassificationLevel, setIndexInArray } from "../utilities/data";
import { htsCodeDigitsOnly, normalizeHtsCode } from "./hts-code";

export const CLASSIFICATION_FROM_HTS_CODE_PLACEHOLDER_DESCRIPTION = "\u2014";
...
export function lacksProductDescriptionForAnalysis(
  ...
  return (
    t.length === 0 || t === CLASSIFICATION_FROM_HTS_CODE_PLACEHOLDER_DESCRIPTION
  );
}
...
export function buildCompletedClassificationFromHtsCode(
  ...
  const progressionDescription = chain
    .map((e) => e.description)
    .join(" > ");

  const classification: ClassificationI = {
    articleDescription: CLASSIFICATION_FROM_HTS_CODE_PLACEHOLDER_DESCRIPTION,
    articleAnalysis: "",
