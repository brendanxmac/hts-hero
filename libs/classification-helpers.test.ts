...
const baseRecord = (overrides: Partial<ClassificationRecord>): ClassificationRecord =>
  ({
    id: "class-1",
    user_id: "user-1",
    team_id: "team-a",
...
describe("canUserDelete", () => {
  it("owner can delete their own draft", () => {
    const profile = baseProfile({ id: "user-1" });
    const record = baseRecord({ user_id: "user-1", status: ClassificationStatus.DRAFT });
    expect(canUserDelete(profile, record)).toBe(true);
  });

  it("owner cannot delete completed classification", () => {
    const profile = baseProfile({ id: "user-1" });
    const record = baseRecord({ user_id: "user-1", status: ClassificationStatus.FINAL });
    expect(canUserDelete(profile, record)).toBe(false);
  });

  it("team admin cannot delete team member's classification", () => {
    const profile = baseProfile({ id: "user-2", role: UserRole.ADMIN, team_id: "team-a" });
    const record = baseRecord({ user_id: "user-1", team_id: "team-a", status: ClassificationStatus.DRAFT });
    expect(canUserDelete(profile, record)).toBe(false);
  });
...
describe("hasPreliminaryLevels", () => {
  it("returns true when preliminaryLevels has items", () => {
    const classification = {
      preliminaryLevels: [{ level: "section" as const, candidates: [], analysis: "" }],
    };
...
describe("shouldSkipSectionChapterDiscovery", () => {
  it("returns true for old classification (levels with selections, no preliminaryLevels)", () => {
    const classification = {
      preliminaryLevels: [],
      levels: [{ selection: { htsno: "1234", description: "x" }, candidates: [] }],
    };
