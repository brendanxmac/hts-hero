...
export default function HtsCodeSetDropdown({
  ...
  const selectedSet = htsCodeSets[selectedIndex];

  const getSelectedSetsTitle = (set: HtsCodeSet) => {
    let titleBase = "";
    if (set.name) {
      titleBase = set.name;
    } else {
      titleBase = `Untitled Set`;
    }
            ...
            <Listbox.Options className="absolute z-10 mt-1 max-h-64 w-full overflow-auto rounded-md bg-base-100 border-2 border-base-content/60 py-1 text-base shadow-lg shadow-base-300 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {htsCodeSets.map((htsCodeSet, index) => (
                <Listbox.Option
                  key={index}
                  className={({ active }) =>
                    `relative cursor-default select-none py-3 px-4 transition-colors duration-200 text-white ${
                      active ? "bg-base-300" : ""
                    }`
                  }
                  value={index}
                        ...
                        <div className="flex flex-col gap-1 flex-1 min-w-0">
                          <div className="w-full flex items-start justify-between">
                            <p className={`text-base font-bold truncate`}>
                              {htsCodeSet.name || "Untitled Set"}
                            </p>
                            <p className="text-sm text-gray-400">
                              Codes: {htsCodeSet.codes.length}
                            </p>
                          </div>

                          <p className="text-sm text-gray-400">
                            Description: {htsCodeSet.description || "N/A"}
                          </p>
