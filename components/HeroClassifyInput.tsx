...
const EXAMPLE_CHIPS = [
  { label: "Brake pads", description: EXAMPLES[0] },
  { label: "Steel bottle", description: EXAMPLES[1] },
  { label: "Cashmere sweater", description: EXAMPLES[2] },
] as const;
...
export function HeroClassifyInput({
  ...
  const isCyclingPaused =
    description.trim().length > 0 || isFocused || isCreating;
  ...
  return (
    <>
      <div className="w-full">
        <div className="relative rounded-xl border-2 bg-base-100/90 backdrop-blur-sm transition-all duration-300 border-base-300 shadow-sm shadow-base-content/10">
          <div className="relative px-5 pt-5 sm:px-6 sm:pt-6">
            ...
            <div className="relative">
              {showPlaceholder && (
                <p
                  aria-hidden="true"
                  className="pointer-events-none absolute inset-0 text-base sm:text-lg md:text-xl leading-relaxed text-base-content/35 text-center motion-reduce:transition-none"
                  style={{
              ...
              <textarea
                ...
                rows={2}
                className="relative z-10 w-full resize-none bg-transparent text-base sm:text-lg md:text-xl leading-relaxed text-base-content placeholder-transparent focus:outline-none text-center"
        ...
        <div className="relative px-4 pb-4 pt-3 sm:px-5 sm:pb-5 sm:pt-4">
          ...
          <div className="mt-3.5 flex flex-wrap items-center justify-center gap-x-2 gap-y-2">
            <span className="text-xs sm:text-sm text-base-content/45">
              Use an example
            </span>
            {EXAMPLE_CHIPS.map((chip) => {
              const isSelected = description === chip.description;
              return (
                <button
                  key={chip.label}
                  type="button"
