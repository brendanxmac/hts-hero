...
export const ToolsDropdown = ({ className = "" }: ToolsDropdownProps) => {
  ...
  // Find the currently selected tool based on URL
  const selectedTool = toolLinks.find((tool) => pathname.startsWith(tool.href));
        ...
        <>
          ...
          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 z-[9999] w-72 sm:w-80 bg-base-100 rounded-2xl shadow-2xl border border-base-content/10 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="relative p-2">
              {toolLinks.map((tool) => {
                const isSelected = pathname.startsWith(tool.href);
                // Extract the base color from hoverBg (e.g., "hover:bg-secondary/10" -> "bg-secondary/10")
                const selectedBg = tool.hoverBg.replace("hover:", "");
                return (
                  <Link
                    key={tool.href}
                    href={tool.href}
